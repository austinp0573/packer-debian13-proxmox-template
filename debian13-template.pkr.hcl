...
build {
  ...
  provisioner "file" { source = "provisioners/10-base.sh"        destination = "/tmp/10-base.sh" }
  provisioner "file" { source = "provisioners/90-cloudinit-clean.sh" destination = "/tmp/90-cloudinit-clean.sh" }
